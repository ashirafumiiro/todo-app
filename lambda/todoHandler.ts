...
const createResponse = (body: string | AWS.DynamoDB.DocumentClient.ItemList,
    statusCode = 200)=>{
        return {
            statusCode,
            headers: {
...
exports.handler = async function (event: AWSLambda.APIGatewayEvent) {
    try {
        ...
        if(httpMethod === 'POST'){
            ...
            return todo ? createResponse(`${todo} added to the database`): createResponse("Todo missing", 500)
        }

        if(httpMethod === 'DELETE'){
            const id = await deleteTodoItem(data);
            return id ? createResponse(`Todo item with id ${id} deleted`)
            : createResponse("Id missing", 500);  
        }
